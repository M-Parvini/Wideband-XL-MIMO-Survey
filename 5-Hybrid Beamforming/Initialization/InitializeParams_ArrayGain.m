function [OFDM, Chan, BS, UE] = InitializeParams_ArrayGain(SNRdB, SCS, RF, Nt, Nr)

OFDM.SNRdB = SNRdB;                 % Signal-to-noise ratio
OFDM.subs = SCS;
OFDM.RFchain = RF;               % Number of RF Chains (NRF)
OFDM.nfft = 256;                % FFT length
OFDM.numOFDMSym = 1;            % Number of OFDM symbols (Ns)
OFDM.BW = OFDM.nfft*OFDM.subs;  % System Bandwidth

%%% Channel parameters %%%
Chan.delay_spread = 500e-9;    % Delay Spread of the channel
Chan.doppler = 0;
Chan.fc = 8e9;                  % center frequency
...
% Chan.pathGains = ExpPDP(OFDM.BW, Chan.delay_spread, Chan.numClusters);
% Chan.pathGains = reNormalize(db2pow(Chan.pathGains));
